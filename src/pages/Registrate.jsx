...
export default function Registrate() {
    ...
    const handle__Submit = (e) => {
        ...
        try {
            axiosJs.post("api/v1/auth/users/", userDate)
                .then(res => {
                    if (res?.data) {
                        localStorage.setItem("my_self", JSON.stringify(res.data.id))
                        window.location.href = "/login"
                    }
                })
                .catch(res => {
                    let key = Object.keys(res.response.data)[0]
                    let value = Object.values(res.response.data)[0][0]

                    setBad(prev => {
                        for (let key1 in prev) {
                            if (key == key1) prev[key1] = value
                            else prev[key1] = ""
                        }
                    ...
                    <div className="w-[425px] flex flex-col">
                        <form onSubmit={handle__Submit} className="w-[100%] flex flex-col">
                            <div className="flex flex-col gap-[5px] mb-[10px]">
                                <p className="font-[500] font-roboto text-[13px] leading-[18px] cursor-pointer text-[#6d6e6f]">Email</p>
                                <label htmlFor="email" className={`flex items-center gap-[10px] border-[1px] border-solid border-[#474d4f] rounded-[3px] px-[10px] cursor-pointer ${bad.email ? "border-b-[3px] border-b-[red]" : ""}`}>
                                    <svg viewBox="0 0 24 24" width="16" height="16">
                                        <path d="M3 3H21C21.5523 3 22 3.44772 22 4V20C22 20.5523 21.5523 21 21 21H3C2.44772 21 2 20.5523 2 20V4C2 3.44772 2.44772 3 3 3ZM20 7.23792L12.0718 14.338L4 7.21594V19H20V7.23792ZM4.51146 5L12.0619 11.662L19.501 5H4.51146Z" fill="#474d4f" />
                                    </svg>
                                    <input className="w-[100%] py-[10px] outline-none font-roboto text-[#222b37] placeholder:font-roboto placeholder:text-[#37414e]" type="email" id="email" name="email" placeholder="Enter yor email...." required onChange={handle__Change} />
                                </label>
                                {
                                    bad.email
                                    &&
                                    <p className="text-[red] font-roboto text-[14px] font-[400]">{bad.email}</p>
                                }
                            </div>
                            <div className="flex flex-col gap-[5px] mb-[10px]">
                                <p className="font-[500] font-roboto text-[13px] leading-[18px] cursor-pointer text-[#6d6e6f]">Name</p>
                                <label htmlFor="name" className={`flex items-center gap-[10px] border-[1px] border-solid border-[#474d4f] rounded-[3px] px-[10px] cursor-pointer ${bad.username ? "border-b-[3px] border-b-[red]" : ""}`}>
                                    <svg viewBox="0 0 24 24" width="16" height="16">
                                ...
                                </label>
                                {
                                    bad.username
                                    &&
                                    <p className="text-[red] font-roboto text-[14px] font-[400]">{bad.username}</p>
                                }
                            </div>
                            <div className="flex flex-col gap-[5px] mb-[10px]">
                                <p className="font-[500] font-roboto text-[13px] leading-[18px] cursor-pointer text-[#6d6e6f]">Password</p>
                                <label htmlFor="password" className={`flex items-center gap-[10px] border-[1px] border-solid border-[#474d4f] rounded-[3px] px-[10px] cursor-pointer ${bad.password ? "border-b-[3px] border-b-[red]" : ""}`}>
                                    <svg viewBox="0 0 24 24" width="16" height="16">
                                ...
                                </label>
                                {
                                    bad.password
                                    &&
                                    <p className="text-[red] font-roboto text-[14px] font-[400]">{bad.password}</p>
                                }
                            </div>
